...
class FormulaPage2 extends StatefulWidget {
  const FormulaPage2({super.key, required this.constraints});

  final BoxConstraints constraints;

  @override
  State<FormulaPage2> createState() => _FormulaPage2State();
}

class _FormulaPage2State extends State<FormulaPage2> {
  // 1是列，2是3d
  int type = 1;
  ...
  Future<List<Map<String, String>>> getData2() async {
    ...
    if (response.statusCode == 200) {
      var r =null;
      if(type==1){
        r=getData1ByHtml(response.data,2)["data"] as List<Map<String, String>>;
      }else{
        r=getData2ByHtml(response.data,2)["data"] as List<Map<String, String>>;
      }
                ...
                children: [
                  TableRow(
                    children: [
                      ...
                      TableCell(child: Center(child: FittedBox(child: Text("公式2")))),
                      TableCell(child: Center(child: FittedBox(child: Text("公式3")))),
                      TableCell(child: Center(child: FittedBox(child: Text("公式4")))),
                      TableCell(child: Center(child: FittedBox(child: Text("公式5")))),
                      TableCell(child: Center(child: FittedBox(child: Text("杀和尾")))),
                    ],
                  )
                ],
              ),
